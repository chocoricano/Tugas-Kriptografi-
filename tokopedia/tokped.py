...
class tokopedia:
    ...
    def dapatkanID(self):
        try:
            req = post(self.link_toko, headers=self.header, timeout=10.000)
        ...
        for num, i in enumerate(self.hasil["data"]["products"]):
            ...
            if self.tampil == 1:
                ...
                self.semua_produk.append(
                    [i["name"], price, i["url"], i["image_url"], i["category_name"]]
                )
            else:
                self.semua_produk.append(
                    [i["name"], price, i["url"], i["image_url"], i["category_name"]]
    ...
    def simpanProduk(self):
        if self.simpan == 1:
            ...
            try:
                df = DataFrame(
                    self.semua_produk,
                    columns=["name", "price", "url", "image_url", "category_name"],
                )
                df.to_csv(
                    f"{self.nama_toko}/{self.nama_toko}_[produk].csv", index=False
                )
            except:
